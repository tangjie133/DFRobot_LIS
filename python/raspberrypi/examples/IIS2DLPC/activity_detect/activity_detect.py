...
'''
      Wake-up duration -- when the setActMode() function uses the detection mode of eDetectAct, it will be a period of time to collect data at a 
...
'''
    Set the mode of motion detection:
                 ...
                 DETECT_ACT        #If set this mode, the rate of the chip will drop to 12.5hz and turn into normal measurement
                                    frequency after the eWakeUp event is generated.
                 DETECT_STATMOTION #In this mode, it can only detect if the chip is in sleep mode without changing the measurement 
                                    frequency and power mode, continuously measuring the data at normal frequency.
'''
